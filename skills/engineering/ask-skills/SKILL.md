...
2. **Prototype if needed** - use **`/handoff`** to branch into **`/prototype`** when a question needs a runnable answer, then hand the result back.
3. **Split or implement** - for multi-session work, use **`/to-prd`** then **`/to-issues`**. For one focused slice, go straight to **`/implement`**.
4. **Build with feedback** - `/implement` drives **`/tdd`**, then closes with **`/code-review`**.
